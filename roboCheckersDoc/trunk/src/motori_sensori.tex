...
\begin{lstlisting}
 ...
 /** restituisce vero se il movimento di rotazione ad 
  un angolo specifico non è stato completato */
 boolean isRotating();
 ...
 /** ruota di un determinato angolo rispetto alla  posizione 
	 ...
	 è bloccante e restituisce subito il controllo completando 
...
per la gestione del sensore di pressione, che è stato utilizzato
per il controllo del movimento su-giù del braccio.\\
\begin{figure}
  \begin{center}
...
\end{lstlisting}
Dove il parametro \texttt{port} è un'implementazione dell'interfaccia
\texttt{ADSensorPort}, ad esempio \texttt{SensorPort}, che indica
a quale porta è connesso il sensore.\\
\\
La classe \texttt{TouchSensor} mette a disposizione un solo metodo:
\begin{lstlisting}
	/** restituisce vero quando il pulsante è premuto */
	boolean isPressed();
\end{lstlisting} 


\subsection{Il sensore di colore}
Il sensore di colore non è fornito nella normale dotazione di
...
indichi a quale porta del brick NXT il sensore è connesso.\\
\\
