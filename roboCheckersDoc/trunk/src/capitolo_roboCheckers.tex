...
Questa interfaccia è usata per uniformare il giocatore robotico, il quale deve
pensare ed effettuare la mossa, con il giocatore umano, di cui, invece, occorre
determinare la mossa effettuata. \\ Il metodo che esegue tutto questo è:
\begin{lstlisting}
...
\end{lstlisting}
Come si può notare dalle eccezioni lanciabili, il metodo permette di
riconoscere il caso in cui non ci siano più mosse disponibili sia da parte del
robot che dell'umano e il caso in cui venga fatta una mossa non valida da parte
dell'umano. Ovviamente tutto ciò non può funzionare se non è prima avvenuta la
calibrazione (\texttt{NotCalibratedException}).

\subsection{ComputerPlayer}
Il suo metodo principale (\texttt{makeMove()}) si può sostanzialmente
suddividere in due parti. Inizialmente viene scelta tra le mosse disponibili
quella che permetterà di ottenere il punteggio migliore, attraverso il metodo
...
e poi la casella in cui è stato scelto di spostarla, questo utilizzando le
istanze \emph{singleTon} delle classi \texttt{CheckersNavigator} e
\texttt{ArmController}.\\ Il livello di intelligenza della macchina è gestito mediante il livello di profondità massimo della ricorsione, eseguita in \texttt{MiniMax()}, e viene impostato attraverso l'attributo \texttt{depth}.
...
\section{Factory}
La classe \texttt{Factory} è stata introdotta per garantire una scelta
...
Bluetooth.
L'input può essere gestito da una qualsiasi delle classi che implementano
