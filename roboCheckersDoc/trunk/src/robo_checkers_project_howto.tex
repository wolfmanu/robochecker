...
Si dà come presupposto di avere già installato Eclipse con il
...
di Lejos.
L'installazione è molto semplice, una volta scaricato il setup dal sito
ufficiale \url{http://lejos.sourceforge.net/} è sufficiente seguire il wizard
fino alla fine. L'unica cosa importante da fare è ricordarsi la cartella in cui
lo si decide di installare in quanto sarà necessaria in seguito.
\section{Installazione del Plugin di Lejos}
Dopo aver installato Lejos è possibile installare il Plugin per Eclipse così da
poter compilare e caricare i programmi sull'\texttt{NXT} direttamente da
Eclipse. Per farlo, selezionare il menù \texttt{Help}$\to$\texttt{Software Updates} e nella sezione
...
successivamente premere \texttt{Install} e procedere con l'installazione.
	...
	\end{figure}
\paragraph{}
Una volta riavviato Eclipse, è necessario configurare il plugin indicando la
cartella dove è stato installato Lejos. Da
...
A questo punto, prima di procedere con il download dei sorgenti, è ancora
necessario un altro piccolo passo per la configurazione dell'ambiente. Questo
si è reso necessario perchè \emph{Lejos} sui vari sistemi operativi è installato
in posizioni diverse quindi al fine di rendere il progetto generico è necessario
specificare alcune variabili per la configurazione corretta del Classpath di
Java.\\
Da \texttt{Window}$\to$\texttt{Preferences} selezionare la sezione
...
all'interno della cartella lib dell'installazione di \emph{Lejos}.\\
	\begin{figure}
		...
		\end{center}
	\end{figure}
Ripetere quindi le istruzioni al passo precedente ed aggiungere altre due
variabili d'ambiente chiamate \texttt{PCCOMM\_CLASSES} e
\texttt{PCTOOLS\_CLASSES} che fanno rispettivamente riferimento ai file
\emph{pccomm.jar} e \emph{pctools.jar}.
Arrivati a questo punto la configurazione dell'ambiente Eclipse è finita ed è
possibile scaricare i sorgenti del progetto.

\section{Checkout dal repository SVN}
Per fare il checkout dei sorgenti basta semplicemente seguire le istruzioni già
utilizzate in precedenza\footnote{fare riferimento al paragrafo
\ref{sec:checkoutsvn}} cambiando semplicemente l'indirizzo che risulta:
\begin{itemize}
\item \url{http://robochecker.googlecode.com/svn/roboChechersNXJ/trunk} per il
progetto base dove è presente il codice per la gestione del gioco e il
programma sequenziale;
\item \url{http://robochecker.googlecode.com/svn/roboChechers/trunk} per il
progetto della macchina a stati che farà riferimento ai sorgenti del progetto
