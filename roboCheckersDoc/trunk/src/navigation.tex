...
il grafico dell'andamento delle coordinate $(\theta,C_y)$ al variare di
\texttt{coeffB} (Figure \ref{theta(CoeffB)} e \ref{Cy(CoeffB)})
...
Al fine di rendere i movimenti del braccio più rapidi, si è manifestata la
necessità di poter eseguire le chiamate ai metodi \texttt{up()} e
...
\end{lstlisting}
Implementare questo tipo di comportamento è banale per il metodo
\texttt{down()}, perchè questo esegue al suo interno una chiamata al metodo
\texttt{Motor.rotateTo()} che le API di Lejos già prevedono in versione non 
bloccante.
\begin{lstlisting}
...
caso è necessario disporre di un meccanismo che controlli periodicamente lo
stato del sensore di pressione e fermi il motore al momento giusto, anche se
\texttt{up()} è già ritornato.\\
A questo scopo è stata progettata la classe annidata \texttt{ArmRegulator}:
\begin{lstlisting}
...
stato del sistema a \texttt{GOUP}, del resto si occuperà \texttt{armRegulator}:
\begin{lstlisting}
