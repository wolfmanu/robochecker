...
singola mossa generate dalla classe \texttt{Engine}, questa strazione viene
implementata dai metodi \texttt{toArray()}, che fa il mapping da oggetto a
vettore di interi, e \texttt{fromArray()},si occupa di astrarre le mosse da
vettore ad oggetto.\\
Nello specifico la classe move salva le mosse come una \texttt{Square}
contenente la casella di partenza ed un vettore di \texttt{Square} contenente
tutte le destinazioni, cioè in caso di prese multiple non sarà presente solo i
valori della casella di partenza e di destinazione finale, ma anche di tutti i
salti intermedi.
...
\section{Board}
\'E la rappresentazione della scacchiera all'interno del bricco. La classe al
suo interno, oltre alla posione attuale delle pedine, contiene una collezione
di \texttt{MoveCollection} in cui sono incluse tutte le possibili mosse
effettuabili dall'avversario del robot. Queste possono essere prelevate mediante
il metodo \texttt{getPossibleMoves()} che le restituisce sotto forma di
\texttt{Vector<MovesCollections>}, oppure, se si necessita delle caselle di
partenza e di arrivo separatamente, sono disponibili i metodi
\texttt{getPossibleMoveFrom()} (restituisce una casella ad ogni chiamata) e
\texttt{getPossibleMoveTo()} (restituisce ad ogni chiamata una delle
destinazioni raggiungibili dall'ultima casella restituita da
\texttt{getPossibleMoveFrom()}).
