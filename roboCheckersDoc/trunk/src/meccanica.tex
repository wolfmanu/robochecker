...
Figura \ref{Base}) e modificato come segue: il motore B, che nello schema originale si occupava di far
ruotare il braccio, continua nella sua funzione, mentre il motore A
trasferisce la sua funzione da quella di far muovere in avanti il braccio a quella di gestire la
trazione del Robot.\\
...
Questo sistema ha però mostrato delle carenze di precisione dovute a torsioni,
...
\begin{itemize} 
  \item Creando un nuovo sistema di aggancio dell'albero motore verticale, in
 		 modo da minimizzarne la flessione. Nello specifico, si è inserita una
  		 ...
  		 \begin{figure}
           \begin{center}
			 \includegraphics[scale=0.7]{img/trazione.png}
...
essendo questi puramente dipendenti dalla natura fisica degli stessi e
non dal sistema di montaggio.
\section{Il Braccio}
Per la costruzione del braccio si è nuovamente presa ispirazione dalle
...
Inoltre, per dare maggior solidità all'intera struttura, si sono aggiunte delle
barre (\emph{Technic Beam}) che 'legano' i due montanti insieme. In tutto ne
sono state montate tre: una sulla punta del braccio, una sul montante anteriore ed una sul
montante posteriore.

\section{Sistema di leve a forbice}

Per la costruzione della parte terminale del braccio si sono dovuti risolvere
due problemi: il primo ha riguardato il modo di generare un moto verticale, che
abbassasse la punta del braccio, a partire da un moto rotatorio. Il secondo, e
strettamente correlato al primo, è stato quello di dare la giusta precisione in
modo che il sensore fosse sempre alla stessa altezza dalla scacchiera. \\

Il primo problema si è deciso di risolverlo tramite un sistema di
leveraggi imperniati a forbice (in seguito per semplicità denominato
...
risolto dall'adozione del pantografo, si è inserito un treno di ingranaggi
...
permettere una stima empirica del comando da impartire al motore.\\
		  ...
		  \end{center}
		\end{figure} 
Il pantografo è inoltre dotato di due sensori: il primo, situato sotto il treno
di ingranaggi (e quindi solidale con il braccio) è il sensore di pressione
\emph{Electric Touch Sensor NXT} che viene premuto dalla prima leva quando il
pantografo è in alto. 
Il secondo è il sensore di colore \emph{NXT Color Sensor} che, posizionato alla
