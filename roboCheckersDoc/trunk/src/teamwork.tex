...
maturata esternamente al Politecnico da alcuni componenti del gruppo è stato
proposto di utilizzare un sistema di controllo versioni, così da gestire
facilmente la scrittura simultanea del codice da parte di più persone. Tra i
vari sistemi disponibile è stato scelto Subversion principalmente perchè quello
conosciuto da più componenti del gruppo.
\section{Subverion}
...
esempio la possibilità di tornare ad uno stato precedente del progetto, nei
casi in cui si raggiungeva un vicolo cieco.
SVN permette di tenere traccia e di controllare i cambiamenti al codice
sorgente, sapendo sempre chi è l'autore della singola modifica. 
Man mano che il software viene sviluppato, è sempre più probabile che versioni distinte 
...
semplicemente bloccando (lock) i file, cosicché solamente uno sviluppatore per
...
permettono a più sviluppatori di modificare lo stesso file nello stesso tempo,
...
Questo consente un immagazzinamento efficiente di più versioni di un file,
purché, come solitamente succede, le modifiche tra una versione e la successiva
